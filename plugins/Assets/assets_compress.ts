...
const audioBitrate = Math.min(
  320,
  Math.max(32, parseInt(process.argv[3], 10) || 128),
);
...
(async () => {
  ...
  fs.mkdirSync(ASSETS_COMPRESSED_DIR, { recursive: true });

  allDirs.forEach((d) => {
    fs.mkdirSync(path.join(ASSETS_COMPRESSED_DIR, d), { recursive: true });
  });
...
async function compressImages(files: fileData[]) {
...
async function compressAudio(files: fileData[]) {
  if (files.length <= 0) return;
...
async function compressOther(files: fileData[]) {
  if (files.length <= 0) return;
...
function runFfmpeg(args: string[]) {
  return new Promise<void>((resolve, reject) => {
    ...
    proc.on("close", (code) =>
      code === 0
        ? resolve()
        : reject(new Error(err.slice(-500) || "ffmpeg error")),
    );
  });
}
...
function validateFile(
  ...
  const compressedSizePercent =
    fs.statSync(compressedFilePath).size / file.size;
